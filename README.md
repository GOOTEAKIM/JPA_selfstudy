...
- 엔티티가 아닌 타입을 한 개 이상의 필드와 매핑할 때 사용
  - ex. Address, Money 등 매피
- 엔티티의 한 속성으로 @Embeddable 적용 타입 사용

- @Embeddable 애노테이션은 클래스에 적용되어, 해당 클래스가 다른 엔티티에 포함될 수 있는 값 타입임을 나타냅니다. 
- 즉, 이 클래스는 독립적인 엔티티가 아니며 다른 엔티티의 일부로 포함됩니다.

---
### @Embedded

- @Embedded 애노테이션은 엔티티 클래스의 필드에 적용되어, 해당 필드가 @Embeddable 클래스를 참조함을 나타냅니다. 
